...
def get_library_paths():
    ...
    # Tenta pegar das configurações do usuário primeiro
    movies_path = ADDON.getSetting('library.movies.path')
    tvshows_path = ADDON.getSetting('library.tvshows.path')
    
    # Se não configurado, usa caminhos ACESSÍVEIS pelo Kodi
    if not movies_path:
        # Usa a pasta profile do addon (sempre acessível)
    ...
    try:
        ...
        for season in seasons:
            ...
            for episode in episodes:
                ...
                plugin_url = (
                    f"plugin://plugin.video.cineroom.lite/?action=find_sources&media_type=tvshow"
...
def clear_library():
    ...
    try:
        ...
        # Remove TODOS os filmes (não apenas do Cineroom)
...
def add_all_movies_to_library(*args, **kwargs):
    ...
    progress = xbmcgui.DialogProgressBG()
    progress.create("Cineroom Lite", "Adicionando filmes...")
...
def add_all_tvshows_to_library(*args, **kwargs):
    ...
    progress = xbmcgui.DialogProgressBG()
    progress.create("Cineroom Lite", "Adicionando séries...")
